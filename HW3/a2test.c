...
static void double_row_major_plus()
{
        ...
        for (int j = 0; j < H; j++) {
                for (int i = 0; i < W; i++) {
                        //fprintf(stderr, "Now Finding row: %i %i \n", i, j);
                        int *p = methods->at(array, i, j);
                        assert(*p == counter);
                        counter++;
                }
        ...
        //     fprintf(stderr, "in 2\n");
        //         counter = 1;
        //         methods->small_map_block_major(array,
...
static void test_methods(A2Methods_T methods_under_test) 
{
        ...
        for (int i = 0; i < W; i++) {
                for (int j = 0; j < H; j++) {
                        ...
                        fprintf(stderr, "Finding: %i %i \n", i, j);
                        
                        unsigned *p = methods->at(array, i, j);
                        
                        assert(*p == n);
                        fprintf(stderr, "Checked: %i %i \n", i, j);
                }
        }
        for (int i = 0; i < W; i++) {
                for (int j = 0; j < H; j++) {
                        unsigned n = 1000 * i + j;
                        //fprintf(stderr, "in second loop: %i %i \n", i, j);
                        unsigned *p = methods->at(array, i, j);
                        assert(*p == n);
                }
        }
